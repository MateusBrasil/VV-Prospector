...
5. **Arquivo único.** `sites/[slug]/[slug].html` autocontido: CSS inline no `<head>`, sem build, dependências permitidas (deploy Vercel, não FTP).
...
1. **Hero**: nome + especialidade, promessa clara em 1 linha, CTA primário (marcar por telefone/e-mail) visível sem rolar, foto do profissional/clínica.
...
3. **Serviços/áreas de atuação**: cards clicáveis — cada card leva à âncora da secção detalhada ou direto ao contacto (`tel:+351...` ou `mailto:`). **Portugal: e-mail + telefone, nunca WhatsApp/wa.me.**
4. **Sobre**: formação e credenciais reais (geram autoridade — nunca cortar).
5. **Oferta estruturada** (quando fizer sentido): transformar "marque uma consulta" em opções de engajamento (ex.: sessão pontual, acompanhamento 90 dias, plano semestral) — SEM preços, apenas nomes e o que incluem, todos levando ao contacto (telefone/e-mail). Só criar planos que sejam agrupamento óbvio do serviço já oferecido.
...
- **1 CTA por dobra**, sempre orientado à ação e ao benefício ("Quero a minha avaliação" > "Clique aqui"), todos para o contacto real (telefone/e-mail).
...
- Barra de CTA fixa no telemóvel (marcar por telefone) — nunca botão de WhatsApp flutuante.
...
- [ ] Todos os links e CTAs apontam para contacto REAL do cliente (`tel:+351...` / `mailto:`)
- [ ] Telefone clicável no formato PT (`tel:+351XXXXXXXXX`) — sem WhatsApp/wa.me
