(function(){
  var __root = document.querySelector('[data-blk="hero-section-hero-14"]');
  if(!__root) return;
...
const body = __root; // Reference to the body element
const frame = __q('.frame'); // Reference to the frame element
const content = __q('.content'); // Reference to the content element
const enterButton = __q('.enter'); // Reference to the "Explore" button
const fullview = __q('.fullview'); // Reference to the fullview element
const grid = __q('.grid'); // Reference to the grid element
...
// Preloading images and initializing setup when complete (com limite de 2.5s para nunca prender o loader)
const __semLoader = new Promise(r => setTimeout(r, 2500));
Promise.race([preloadImages('.row__item-img'), __semLoader]).then(() => {
