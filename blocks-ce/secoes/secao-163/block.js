(function(){
  var __root = document.querySelector('[data-blk="secoes-secao-163"]');
  if(!__root) return;
...
function initScrollReveal() {
  ...
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    __qa("[data-reveal], [data-reveal-slide], .scroll-slide-left").forEach((el) => {
      el.style.opacity = "1";
      el.style.transform = "none";
    });
    __qa("[data-reveal-image] .img-wrapper").forEach((el) => {
      el.style.transform = "translate3d(0,0,0)";
    });
    __qa("[data-reveal-image] .img").forEach((el) => {
