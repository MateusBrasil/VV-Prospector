(function(){
  var __root = document.querySelector('[data-blk="menu-navegacao-menu-21"]');
  if(!__root) return;
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const menu = __q('.circular-menu')
	const joystick = __q('.joystick')
	const menuOverlayNav = __q('.menu-overlay-nav')
	const menuOverlayFooter = __q('.menu-overlay-footer')
	...
	__q('.close-btn').addEventListener('click', toggleMenu)
...
function toggleMenu() {
	...
	const menuSegments = __qa('.menu-segment')
	const joystick = __q('.joystick')
	const menuOverlayNav = __q('.menu-overlay-nav')
	const menuOverlayFooter = __q('.menu-overlay-footer')
	...
	function animate() {
		...
		if (
			...
			const segmentIndex =
				...
				menuItems.length
			const segment = __qa('.menu-segment')[segmentIndex]
