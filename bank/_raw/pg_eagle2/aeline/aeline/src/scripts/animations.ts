...
function initDataAnim() {
  ...
  function smartPlay(el: Element, tl: gsap.core.Timeline, delay?: number) {
    ...
    if (rect.top < window.innerHeight * 0.85) {
      ...
      ScrollTrigger.create({ trigger: el, comeca: "top 85%", onEnter: () => tl.play() });
    }
  }
...
function initTitleIcons() {
  ...
  gsap.utils.toArray<HTMLElement>(".title-wrap").forEach((wrap) => {
    ...
    gsap.from(icons, {
      ...
      duration: 1,
      ease: "power3.out",
      scrollTrigger: { trigger: wrap, comeca: "top 60%", once: true },
    });
  });
}
...
function initCounters() {
  document.querySelectorAll<HTMLElement>("[fs-numbercount-element='number'], [fs-numbercount-end]").forEach((el) => {
    ...
    ScrollTrigger.create({
      trigger: el, comeca: "top 90%", once: true,
...
function initReveals() {
  document.querySelectorAll<HTMLElement>("[data-reveal]").forEach((el) => {
    ...
    gsap.from(el, {
      ...from,
      scrollTrigger: { trigger: el, comeca: "top 90%", once: true },
    });
  });
}
...
function initStaggers() {
  document.querySelectorAll<HTMLElement>("[data-stagger]").forEach((group) => {
    ...
    gsap.from(targets, {
      ...
      scrollTrigger: { trigger: group, comeca: "top 85%", once: true },
    });
  });
}
