...
- Pages identified (Inicio, Sobre, Contato, etc.)
- Sections per page (Hero, Recursos, Depoimentos, Footer, etc.) — **each section MUST include its `nodeId`** so Phase 2 agents can call `get_design_context` and `get_screenshot` directly
...
1. **Download ALL images using a Node.js script** — do NOT use `curl` or `wget` as these may be blocked by shell permissions. Use the following pattern to batch-download all images in a single `node -e` command:
   ...
   Promise.all(images.map(([n, u]) => download(n, u))).then(results => {
     resultados.forEach(r => console.log(r.status + ' | ' + r.name + ' | ' + r.size + ' bytes'));
   });
   "
   ```
...
The SITE_MAP.md MUST include Figma references so Phase 2 agents can call Figma MCP tools directly. Without these, agents will build from text descriptions and produce inaccurate resultados.
