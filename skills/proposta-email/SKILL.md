...
- **Assinatura**: nome, apresentação, telefone (`+351...`) e e-mail do config (assinatura completa humaniza e reduz suspeita). Portugal: e-mail + telefone, nunca WhatsApp.
...
- [ ] **Domínio limpo e humano.** Se o link for um subdomínio técnico/temporário (cheio de números, tipo `prospector-slug-a1b2c3.vercel.app`), PARE antes de enviar qualquer proposta: link assim parece golpe e mata a confiança que a capa constrói. Para o piloto, um `.vercel.app` limpo passa; para cliente pagante, oriente a ativar o domínio próprio (`.pt`) e atualizar o campo `dominio` no `prospector-config.json`. Proposta só sai com domínio apresentável.
...
- Nunca enviar para lead sem e-mail confirmado; nesses casos, sugerir contacto por telefone (`+351`) com a mesma abordagem adaptada. Portugal: sem WhatsApp.

## Página-capa (o que o cliente vê ao clicar)

O link do e-mail leva à página-capa gerada por `tools/capa.mjs`: nome do cliente no topo, antes/depois lado a lado e a assinatura do utilizador. Ela existe para dar credibilidade ao clique — o cliente vê o próprio negócio, não um link estranho. Exigências: servida em `https://`, personalizada com dados reais, sem pedido de dado pessoal nenhum.

## Depois do envio

Registar o estado no `prospector.db` via `node tools/crm.mjs status <slug> proposta_enviada` (a `data_proposta` preenche-se sozinha). As respostas são verificadas pelo `/respostas` (Gmail via conector) — sugira agendar a verificação diária. Follow-up pelo `/followup` após 3+ dias sem resposta (1 único follow-up por lead: curto, gentil, "conseguiu ver a página?").
