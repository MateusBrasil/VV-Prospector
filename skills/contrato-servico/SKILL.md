...
> ⚠️ **PENDÊNCIA REGISTADA NO README (secção "Diferenças vs. o upstream" / pendências).** Esta skill está em formato **Brasil**: pede **CPF/CNPJ**, valores em reais, e sugere assinatura via **gov.br/Autentique** (serviços brasileiros). Para um cliente **português** é preciso reescrever para **NIF, RGPD e assinatura qualificada PT**, a fazer com o `legal-squad` (Patrícia Peck + Manuel Lopes Rocha). Não gerar contrato para cliente PT com esta versão sem revisão jurídica manual completa.
