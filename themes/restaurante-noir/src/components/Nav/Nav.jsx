...
const ROTAS_HOME = [
  { href: "/", omissivel: null },
  { href: "/about", omissivel: "paginas.sobre" },
  { href: "/menu", omissivel: "paginas.menu" },
  { href: "/reservation", omissivel: "paginas.reserva" },
];
...
const NAV_LINKS = (blocos.nav?.links ?? [])
  .map((link, index) => ({
    ...link,
    ...ROTAS_HOME[index],
    // RECUO QUE FALTAVA: `blocos.nav.links[i].imagem` é OPCIONAL (a foto de pré-visualização
    // de cada link, ao passar o rato), mas `imagens.hero` é FATAL (existe sempre). Sem este
    // recuo, um cliente que não dê imagem por link produzia `<img src={undefined}>`, o React
    // omitia o atributo `src` de vez, e o menu fullscreen ficava com uma imagem quebrada em
    // toda página do site. Achado ao construir um segundo tema (moda-editorial) onde o
    // `cliente.json` só dava `{label, href}` por link, sem foto própria por item.
