...
const Testimonials = () => {
  ...
  useEffect(() => {
    ...
    // GUARDA QUE FALTAVA (achada num cliente real sem `depoimentos`, tema moda-editorial):
    // quando a secção falta, o componente devolve `null` no fim, mas os HOOKS já correram
    // antes disso, porque em React um hook corre sempre, aconteça o que acontecer no
    // return. Sem isto, `track` é `null` e `track.querySelectorAll` rebentava a
    // hidratação da página INTEIRA, não só desta secção. O efeito de cima já tinha a
    // guarda certa; faltava só aqui.
    if (!track) return;
