...
/* Saiu daqui: o título e o lead do hero, o telefone (contactos.telefone) e o link de
 * reserva do botão (contactos.reservaUrl, DERIVADO de contactos.whatsapp), e os
 * INFO_CARDS inteiros (agora paginas.reserva.cards). Os ícones ficam no tema: o JSON
 * guarda só uma chave ("relampago"/"brilho"/"lua"), mapeada aqui para o componente. */
...
import { contactos, paginas } from "@/theme/content";
...
const ICONES = { relampago: HiBolt, brilho: HiSparkles, lua: HiMoon };

const INFO_CARDS = paginas.reserva.cards.map((card) => ({
  icon: ICONES[card.icone],
  title: card.titulo,
      ...
      <section className="reservation-hero" ref={heroSectionRef}>
        <div className="container">
          ...
          <Copy type="lines" animateOnScroll={false} delay={1.35}>
            <p className="lg">{paginas.reserva.lead}</p>
          </Copy>

          <div className="reservation-hero-button" ref={heroButtonRef}>
            <Button href={contactos.reservaUrl}>{paginas.reserva.botao}</Button>
          </div>

          <Copy type="lines" animateOnScroll={false} delay={1.65}>
            <p className="mono">( {contactos.telefone} )</p>
          </Copy>
        </div>
      </section>
