"use client";
/* DOBRA gerada pela esteira a partir de bank/_componentes/cursor/efeitos-mouse-10
 *
...
export default function Dobra({ slots: s = {} }) {
  ...
  //             ? 1
  //             : (1 / this.options.totalTrailElements) * i + 1 / this.options.totalTrailElements;
  //   
  ...
  //           map(cursor.x, 0, winsize.width, this.options.valuesFromTo.x[0], this.options.valuesFromTo.x[1]),
  //           amt
  //         );
  //         this.imgTransforms[i].y = lerp(
  //           this.imgTransforms[i].y,
  //           map(cursor.y, 0, winsize.height, this.options.valuesFromTo.y[0], this.options.valuesFromTo.y[1]),
  //           amt
  //         );
  //   
  //         this.imgTransforms[i].rz = lerp(
  //           this.imgTransforms[i].rz,
  //           map(cursor.x, 0, winsize.width, this.options.valuesFromTo.rz[0], this.options.valuesFromTo.rz[1]),
  //           amt
  //         );
  ...
  //               map(cursor.y, 0, winsize.height, this.options.valuesFromTo.rx[0], this.options.valuesFromTo.rx[1]),
  //               amt
  //             );
  ...
  //               map(cursor.x, 0, winsize.width, this.options.valuesFromTo.ry[0], this.options.valuesFromTo.ry[1]),
  //               amt
  //             );
  //   
  ...
  return (
    <section className="dobra" data-dobra="cursor-efeitos-mouse-10" ref={raiz}>
      <main>
      
          <div className="content">
            <div className="trail" style={{backgroundImage: `url(${s.imagem})`}}></div>
          </div>
        </main>
    </section>
  );
