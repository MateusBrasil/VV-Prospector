...
const nextConfig = {
  ...
  /* CORRIGIDO: sem isto, `next build` escreve "sobre.html" na raiz de `out/` MAS também
   * cria uma pasta `sobre/` (com os payloads internos do Next para navegação client-side).
   * Numa hospedagem estática (Vercel incluído), pedir "/sobre" encontra a PASTA sem
   * `index.html` dentro dela e devolve 404 — a página existe (sobre.html), só não no
   * caminho que qualquer link/URL direto pede. Confirmado ao vivo: /sobre e /contacto da
   * Vaninha Fashion devolviam 404 real, mesmo com o site publicado e a home funcionando.
   * `trailingSlash:true` faz o export escrever `sobre/index.html` em vez de `sobre.html`
   * — mesmo caminho que a pasta já existia para. Bug sistémico do motor (afeta qualquer
   * tema com mais de uma página), não só este cliente. */
  trailingSlash: true,
