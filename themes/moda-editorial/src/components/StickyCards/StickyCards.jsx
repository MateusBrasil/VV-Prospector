...
const StickyCards = () => {
  ...
  useEffect(() => {
    ...
    // GUARDA QUE FALTAVA (mesmo defeito do Testimonials, ver o comentário lá): quando
    // `blocos.stickyCards` falta, o componente devolve `null` no fim (linha ~93), mas o
    // hook já correu antes disso. Sem isto, `section` é `null` e `section.querySelectorAll`
    // dentro de `buildScrollAnimation` rebenta a hidratação da página inteira.
    if (!section) return;
    let ctx;
