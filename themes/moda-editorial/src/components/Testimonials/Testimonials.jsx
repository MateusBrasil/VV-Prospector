...
/* Saiu daqui: os depoimentos (agora lidos de @/theme/content, `testimonials-data.js`
 * foi apagado) e o título "O que dizem as avaliações" (blocos.testimonials.titulo).
 * Secção some se `depoimentos` faltar. */
...
import { blocos, depoimentos, tem } from "@/theme/content";
...
const Testimonials = () => {
  ...
  useEffect(() => {
    ...
    // GUARDA QUE FALTAVA, e o defeito não era só desta cópia: quando `depoimentos` falta
    // (secção OMISSÍVEL por desenho, ver tools/tema/schema.mjs), o componente devolve
    // `null` no fim (linha ~215), mas os HOOKS já correram antes disso, porque em React
    // um hook corre sempre, aconteça o que acontecer no return. Sem esta guarda, `track`
    // é `null` (nada foi montado) e `track.querySelectorAll` rebenta com "Cannot read
    // properties of null", derrubando a hidratação da página inteira, mesmo em rotas
    // que não têm nada a ver com depoimentos. O 1º useEffect desta dobra já tinha a
    // guarda certa (linha ~45); faltava só aqui.
    if (!track) return;
  ...
  if (!tem("depoimentos")) return null;
      ...
      <div className="container">
        <div className="testimonials-header">
          <Copy type="lines" animateOnScroll>
            <h3>{blocos.testimonials.titulo}</h3>
          </Copy>
            ...
            <div className="testimonial-card" key={index}>
              ...
              <div className="testimonial-author">
                ...
                <div className="testimonial-author-info">
                  ...
                  <StarRating count={testimonial.nota} />
                </div>
              </div>
            </div>
          ))}
