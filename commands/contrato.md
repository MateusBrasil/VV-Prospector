...
> ⚠️ **PENDÊNCIA REGISTADA NO README (secção "Diferenças vs. o upstream" / pendências).** Este comando e a skill `contrato-servico` ainda estão em formato **Brasil** (CPF/CNPJ, gov.br/Autentique, valores em reais) e o mercado deste fork é **Portugal**. Antes de gerar um contrato para um cliente **português**, é preciso reescrever para **NIF, RGPD e assinatura qualificada PT**, trabalho a fazer com o `legal-squad` (Patrícia Peck + Manuel Lopes Rocha). Até lá, NÃO uses este comando para clientes PT sem revisão jurídica manual completa. O resto do funil (prospeção → site → capa → e-mail) já está em PT-PT.
