# Camada de edição visual (legado)

> **Movido em 2026-07-23** de `skills/redesign-premium/references/` (skill arquivada, ver `_ARQUIVO/redesign-premium-html-manual/LEIA-ME.md`) para `referencias/` na raiz.
>
> **Superseded.** Este documento descreve a doutrina antiga: pedir ao LLM que copie o HTML à mão e injete o script abaixo, o que pagava a montagem da página uma segunda vez (~7 min/cliente) só para gerar o editor. `tools/editor.mjs` faz exatamente este trabalho por código, com um script próprio (`pp-editor-*`), zero tokens de LLM, e é o que `/editor` chama hoje (ver `commands/editor.md`). Este ficheiro fica só como referência histórica do problema e da camada original; para a implementação viva, ler `tools/editor.mjs`.
